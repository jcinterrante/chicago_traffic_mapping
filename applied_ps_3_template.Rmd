...
chi_b_time <- chi_b%>%
  ...
  group_by(event_date_time)%>%
  filter(type == "JAM"|
         type == "ACCIDENT")%>%
  summarise(number = n())%>%
  ggplot(mapping=aes(x=hour(event_date_time), y=number))+
  geom_col()+
...
chi_b_all <- chi_b%>%
  ...
  ggplot(mapping=aes(x=hour(event_date_time), fill = type))+
  geom_bar(position = "dodge")+
  labs(x = "Time of Day", y= "Number of Alerts", title = "Number of Alerts by Time of Day for Bus Route 79")
...
chi_c_all <- chi_c%>%
  ...
  ggplot(mapping=aes(x=hour(event_date_time), fill = type))+
  geom_bar(position = "dodge")+
  labs(x = "Time of Day", y= "Number of Alerts", title = "Number of Alerts by Time of Day for Bus Route 66")
chi_c_all
