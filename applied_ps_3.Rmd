...
event_data <- event_data%>%
  mutate("corridor" = ifelse(street %in% c("E 79th St", "W 79th St"), 
                            "79th St", 
                            "Chicago Ave"),
...
time_range <- c(start = convert_to_millis("2017-12-24 00:00:00"), 
                end = convert_to_millis("2017-12-25 00:00:00"))
time_range
#Event date: 2017-12-24 12:02:55 CST
single_event_data <- chi_alerts_sql %>%
  filter(1514095200000 <= pubMillis, pubMillis<= 1514181600000,
...
single_event_data <- single_event_data%>%
  mutate(event_date_time = force_tz(as.POSIXct (pubMillis/1000, origin="1970-01-01")), "America/Chicago")
# OlsonNames() "America/Chicago" 
single_event_bounds <- c(left = -87.624138, bottom = 41.855 , right =
  -87.6, top = 41.89)

single_event_stamenmap <- get_stamenmap(data = single_event_data, 
...
ggplot(aes(event_date_time))+
  geom_freqpoly(bins = 52) +
  geom_vline(xintercept = ymd_hms("2017-Dec-24 11:50:00", tz = "America/Chicago")) +
  geom_vline(xintercept = ymd_hms("2017-Dec-24 15:20:00", tz = "America/Chicago"))
```
...
```{r}
single_event_data <- single_event_data%>%
  mutate(jam_severity = case_when(subtype == "JAM_LIGHT_TRAFFIC" ~ 1,
...
multiple_event_data <- chi_alerts_sql %>%
  filter(-87.66225 <= location_x & location_x <= -87.65935,
...
multiple_event_data <- multiple_event_data%>%
  mutate(event_date_time = as.POSIXct (pubMillis/1000, origin="1970-01-01", tz = "America/Chicago"))

multiple_event_bounds <- c(left = -87.66225, bottom = 42.0017 , right =
  -87.65935, top = 42.0046)

multiple_event_stamenmap <- get_stamenmap(data = multiple_event_data, 
...
```{r}
#categories <- minutes(seq(-120,120,5))
#categories
#library(forcats)
#factor(minutes(5), levels = categories)
#class(minutes(5))


#class(minutes(seq(-120,120,5)))

#factor(1, levels = c(1:3))

categories <- c(-24:24)
multiple_event_data%>%
mutate(event_epoch_millis = pubMillis - convert_to_millis(event_date_time), 
  ...
  geom_col() + 
  labs(title = "Timing of Events around Major Accidents",
...
#  time_bounds <- convert_to_millis(date_time)	+ c(-7200000, 7200000)
#  x_bounds <- longitude + c(-.1 / 69,.1 / 69)
#  y_bounds <- latitude + c(-.1 / 69,.1 / 69)
  ...
  cat("/ntime_min:",time_min,"/ntime_max:",time_max,"/nxmin:",x_min)
  multiple_event_data <- chi_alerts_sql %>%
    filter(time_min <= pubMillis, pubMillis<= time_max,
  ...
  multiple_event_data%>%
  mutate(event_epoch_millis = pubMillis - convert_to_millis(date_time), 
        bucket = factor(event_epoch_millis %/% 300000, levels = categories))%>%
          group_by(bucket)%>%
          summarize(count = n()) %>%
          complete(bucket, fill = list(count = 0))
...
get_alert_summary("df0ae6f2-22ae-3977-8168-0290610801f9",
...
#subset_list<-list(uuid = subset$uuid, 
 #                 date_time = subset$event_date_time, 
  #                latitude = subset$location_y, 
   #               longitude = subset$location_x)


#get_alert_summary(uuid = subset$uuid, 
 #                 date_time = subset$event_date_time, 
  #                latitude = subset$location_y, 
   #               longitude = subset$location_x)
test <- list("df0ae6f2-22ae-3977-8168-0290610801f9",
                  as.POSIXct (1511837969275/1000, origin="1970-01-01"),
                  41.8421,
                  -87.61015)
...
#for(i in seq_along(1:nrow(subset))){
 # subset_test[i] <-list(get_alert_summary(uuid = subset$uuid[i], 
  #                                  date_time = subset$event_date_time[i], 
   #                                 latitude = subset$location_y[i], 
    #                                longitude = subset$location_x[i]))
#}

#combined_table <- colnames(subset_test)

#for(i in seq_along(1:length(subset_test))){
 # combined_table <- bind_rows(combined_table, subset_test[[i]])
#}

#combined_table %>% group_by(bucket)%>%
 # summarize(mean(count))

alert_summary <- ""
for(i in seq_along(1:nrow(multiple_event_data))){
  alert_summary[i] <-list(get_alert_summary(uuid = multiple_event_data$uuid[i], 
...
full_event_summary <- combined_table %>% group_by(bucket)%>%
